...
@section('content')
        ...
        <div class="card-header border-0 pt-6">
          ...
          <div class="card-toolbar">
            <div class="d-flex justify-content-end" data-kt-customer-table-toolbar="base">
              ...
              <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#kt_modal_add_sub_kegiatan">Tambah Sub
                Kegiatan</button>
            </div>
          ...
          @if ($data->isEmpty())
            <div class="alert alert-warning d-flex align-items-center p-5 rounded">
              ...
              <div class="d-flex flex-column">
                ...
                <span>Tidak ada data Sub Kegiatan yang ditemukan.</span>
              </div>
            </div>
          @else
            <table id="kt_sub_kegiatan_table" class="table table-striped table-row-bordered gy-5 gs-7">
              <thead>
                <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                  <th class="w-10px pe-2">
                    <div class="form-check form-check-sm form-check-custom form-check-solid me-3">
                      <input class="form-check-input" type="checkbox" data-kt-check="true"
                        data-kt-check-target="#kt_sub_kegiatan_table .form-check-input" value="1" />
                    </div>
                  </th>
                  ...
                  @foreach ($bidangGrouped as $bidangGroup)
                    ...
                    @foreach ($programGrouped as $programGroup)
                      @php
                        ...
                        $kegiatanGrouped = $programGroup->groupBy('id_kegiatan');
                      @endphp
                          ...
                          <tr>
                            <td>
                              <div class="form-check form-check-sm form-check-custom form-check-solid">
                                <input class="form-check-input" type="checkbox" value="{{ $subKegiatan->id }}" />
                              </div>
                            </td>
                            ...
                            <td>
                              <div class="d-flex justify-content-end">
                                <a href="{{ route('sub-kegiatan.edit', $subKegiatan->id) }}"
                                  class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1" title="Edit Sub Kegiatan">
                                  <i class="ki-outline ki-pencil fs-2"></i>
                                </a>
                                <form action="{{ route('sub-kegiatan.destroy', $subKegiatan->id) }}" method="POST" class="d-inline delete-form">
                                  @csrf
                                  @method('DELETE')
                                  <button type="submit" class="btn btn-icon btn-bg-light btn-active-color-danger btn-sm delete-btn"
                                    title="Hapus Sub Kegiatan" data-name="{{ $subKegiatan->nama_sub_kegiatan }}">
            ...
            <div class="scroll-y me-n7 pe-7" id="kt_modal_add_sub_kegiatan_scroll">
              ...
              <div class="fv-row mb-7">
                ...
                <input type="text" class="form-control form-control-solid @error('kode_sub_kegiatan') is-invalid @enderror"
                  placeholder="Masukkan kode sub kegiatan" name="kode_sub_kegiatan" value="{{ old('kode_sub_kegiatan') }}" maxlength="20"
                  required />
  ...
  <script>
    document.addEventListener("DOMContentLoaded", function() {
      var table = $('#kt_sub_kegiatan_table').DataTable({
        responsive: true,
        searchDelay: 500,
        processing: true,
        serverSide: false,
      ...
      $('#kt_sub_kegiatan_table').on('click', 'tr.group', function() {
        var colIdx;
      ...
      // === Bulk selection ===
      const checkboxes = document.querySelectorAll('#kt_sub_kegiatan_table tbody input[type="checkbox"]');
      const masterCheckbox = document.querySelector('#kt_sub_kegiatan_table thead input[type="checkbox"]');
      ...
      function updateToolbar() {
        const checkedBoxes = document.querySelectorAll('#kt_sub_kegiatan_table tbody input[type="checkbox"]:checked');
      ...
      document.getElementById('bulk_delete_btn')?.addEventListener('click', function() {
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            form.method = 'POST';
            form.action = '{{ route('sub-kegiatan.bulk-delete') }}';
