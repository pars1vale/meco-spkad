<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const table = $('#kt_program_table').DataTable({
      responsive: true,
      searchDelay: 500,
      processing: true,
      serverSide: false,
    ...
    const deleteHandler = new DeleteHandler({
      tableId: 'kt_program_table',
      deleteRoute: '{{ route('program.bulk-delete') }}',
      deleteMessage: 'Data program'
    });
    deleteHandler.init();

    // Checkbox logic
    initCheckboxLogic('kt_program_table');
  });
</script>
