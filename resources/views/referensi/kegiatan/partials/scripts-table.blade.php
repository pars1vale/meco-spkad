<script>
  document.addEventListener("DOMContentLoaded", function() {
    ...
    const table = $('#kt_kegiatan_table').DataTable({
      responsive: true,
      searchDelay: 500,
      processing: true,
      serverSide: false,
    ...
    const deleteHandler = new DeleteHandler({
      tableId: 'kt_kegiatan_table',
      deleteRoute: '{{ route('kegiatan.bulk-delete') }}',
      deleteMessage: 'Data kegiatan'
    });
    deleteHandler.init();

    // Checkbox logic
    initCheckboxLogic('kt_kegiatan_table');
  });
</script>
