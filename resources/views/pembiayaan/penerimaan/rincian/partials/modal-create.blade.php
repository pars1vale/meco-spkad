<div class="modal fade" id="modal_tambah_penerimaan" tabindex="-1" aria-labelledby="modal_tambah_penerimaan_label" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">

      {{-- Header --}}
      <div class="modal-header">
        <h5 class="modal-title fw-bold" id="modal_tambah_penerimaan_label">
          <i class="ki-outline ki-plus-circle fs-3 text-primary me-2"></i>
          Tambah Data Pembiayaan penerimaan
        </h5>
      ...
      {{-- Body --}}
      <form id="form_tambah_penerimaan" method="POST" action="{{ route('pembiayaan.penerimaan.store', $id_skpd) }}">
        @csrf
        ...
        <div class="modal-body">
          ...
          <div class="fv-row mb-6">
            ...
            <select name="id_akun" id="select_id_akun_penerimaan" class="form-select form-select-solid @error('id_akun') is-invalid @enderror"
            ...
            @enderror
            <div class="form-text text-muted fs-7">Pilih kode rekening pembiayaan penerimaan yang sesuai.</div>
          </div>

          {{-- Keterangan / Uraian --}}
          <div class="fv-row mb-6">
            <label class="fw-semibold fs-6 mb-2">Keterangan</label>
            <textarea name="keterangan" id="input_keterangan_penerimaan" class="form-control form-control-solid @error('keterangan') is-invalid @enderror"
              rows="3" placeholder="Masukkan keterangan / uraian pembiayaan penerimaan...">{{ old('keterangan') }}</textarea>
            @error('keterangan')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          ...
          <div class="fv-row mb-2">
            ...
            <div class="input-group input-group-solid">
              ...
              <input type="text" name="nilai" id="input_nilai_penerimaan"
            ...
            <div class="form-text text-muted fs-7 mt-1">
              <span id="preview_nilai_penerimaan" class="text-gray-700 fw-semibold"></span>
            </div>
            <div class="form-text text-muted fs-7">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" id="btn_simpan_penerimaan" class="btn btn-primary fw-semibold">
...
<script>
  ...
  var KTModalTambahpenerimaan = function() {
    ...
    function updatePreview(raw) {
      var preview = document.getElementById('preview_nilai_penerimaan');
      if (!preview) return;
    ...
    return {
      init: function() {
        var inputNilai = document.getElementById('input_nilai_penerimaan');
        ...
        if (inputNilai) {
          ...
          var form = document.getElementById('form_tambah_penerimaan');
          if (form) {
            form.addEventListener('submit', function() {
              inputNilai.value = toNumber(inputNilai.value);
              var btn = document.getElementById('btn_simpan_penerimaan');
              if (btn) {
        ...
        var modalEl = document.getElementById('modal_tambah_penerimaan');
        if (modalEl) {
          modalEl.addEventListener('hidden.bs.modal', function() {
            var form = document.getElementById('form_tambah_penerimaan');
            if (form) form.reset();

            var sel = $('#select_id_akun_penerimaan');
