{{-- ════════════════════════════════════════════════════════════════════
     ...
     Include di rincian.blade.php dengan:
     @include('pendapatan.modal-create-rincian')
════════════════════════════════════════════════════════════════════ --}}
<div class="modal fade" id="modal_tambah_pendapatan" tabindex="-1" aria-labelledby="modal_tambah_pendapatan_label" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg">
    <div class="modal-content">

      {{-- Header --}}
      <div class="modal-header">
        <h5 class="modal-title fw-bold" id="modal_tambah_pendapatan_label">
      ...
      {{-- Body --}}
      <form id="form_tambah_pendapatan" method="POST" action="{{ route('pendapatan.store', $id_skpd) }}">
        @csrf
        ...
        <div class="modal-body">
          ...
          <div class="fv-row mb-6">
            ...
            <textarea name="keterangan" id="input_keterangan" class="form-control form-control-solid @error('keterangan') is-invalid @enderror" rows="3"
              placeholder="Masukkan keterangan / uraian pendapatan...">{{ old('keterangan') }}</textarea>
            @error('keterangan')
              <div class="invalid-feedback">{{ $message }}</div>
            @enderror
          ...
          <div class="fv-row mb-2">
            ...
            <div class="input-group input-group-solid">
              ...
              <input type="text" name="nilai" id="input_nilai" class="form-control form-control-solid @error('nilai') is-invalid @enderror"
                placeholder="0" value="{{ old('nilai') }}" inputmode="numeric" required />
              @error('nilai')
                <div class="invalid-feedback">{{ $message }}</div>
              @enderror
            </div>
        ...
        <div class="modal-footer">
          ...
          <button type="submit" id="btn_simpan_pendapatan" class="btn btn-primary fw-semibold">
...
<script>
  ...
  var KTModalTambahPendapatan = function() {
    ...
    return {
      init: function() {
        ...
        if (inputNilai) {
          ...
          var form = document.getElementById('form_tambah_pendapatan');
          if (form) {
            form.addEventListener('submit', function() {
              inputNilai.value = toNumber(inputNilai.value);
              var btn = document.getElementById('btn_simpan_pendapatan');
              if (btn) {
        ...
        var modalEl = document.getElementById('modal_tambah_pendapatan');
        if (modalEl) {
          modalEl.addEventListener('hidden.bs.modal', function() {
            var form = document.getElementById('form_tambah_pendapatan');
            if (form) form.reset();

            var sel = $('#select_id_akun');
