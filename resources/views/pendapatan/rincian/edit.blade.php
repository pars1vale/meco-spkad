      ...
      <div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100">
        <div class="page-title d-flex flex-column justify-content-center gap-1 me-3">
          ...
          <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0">
            ...
            <li class="breadcrumb-item text-muted">
              <a href="{{ route('pendapatan.rincian', $id_skpd) }}" class="text-muted text-hover-primary">Rincian</a>
            </li>
            <li class="breadcrumb-item">
              <span class="bullet bg-gray-400 w-5px h-2px"></span>
            </li>
  ...
  <div id="kt_app_content" class="app-content flex-column-fluid">
    <div id="kt_app_content_container" class="app-container container-fluid">
      <div class="card">
          ...
          <div class="card-title">
            <h2>Edit Pendapatan:
              <span class="text-primary">
                {{ $pendapatan->kode_akun ?? ($pendapatan->rekening ?? '-') }}
              </span>
            </h2>
          </div>
          <div class="card-toolbar">
            <a href="{{ route('pendapatan.rincian', $id_skpd) }}" class="btn btn-secondary">
        ...
        {{-- Form --}}
        <form action="{{ route('pendapatan.update', [$id_skpd, $pendapatan->id]) }}" method="POST" class="form" id="form_edit_pendapatan">
          @csrf
          @method('PUT')
            ...
            <div class="row">
              ...
              <div class="col-md-6">
                ...
                <div class="fv-row mb-7">
                  ...
                  <select name="id_akun" id="select_id_akun" class="form-select form-select-solid @error('id_akun') is-invalid @enderror"
                    data-control="select2" data-placeholder="-- Pilih Akun --" data-allow-clear="true" required>
                    <option></option>
                    @foreach ($akunList as $akun)
                      <option value="{{ $akun->id }}" {{ old('id_akun', $pendapatan->id_akun) == $akun->id ? 'selected' : '' }}>
                  ...
                  @enderror
                  <div class="form-text text-muted fs-7">Pilih kode rekening pendapatan yang sesuai.</div>
                </div>
                ...
                <div class="fv-row mb-7">
                  ...
                  <div class="input-group input-group-solid">
                    ...
                    <input type="text" name="nilai" id="input_nilai" class="form-control form-control-solid @error('nilai') is-invalid @enderror"
                      placeholder="0" value="{{ old('nilai', $pendapatan->total) }}" inputmode="numeric" required />
                    @error('nilai')
                      <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                  </div>
                  <div class="form-text text-muted fs-7 mt-1">
                    <span id="preview_nilai" class="text-gray-700 fw-semibold"></span>
                  </div>
              ...
              <div class="col-md-6">
                ...
                <div class="fv-row mb-7">
                  ...
                  <textarea name="keterangan" id="input_keterangan" class="form-control form-control-solid @error('keterangan') is-invalid @enderror" rows="5"
                    placeholder="Masukkan keterangan / uraian pendapatan...">{{ old('keterangan', $pendapatan->keterangan) }}</textarea>
                  @error('keterangan')
                    <div class="invalid-feedback">{{ $message }}</div>
                  @enderror
                  <div class="form-text text-muted fs-7">
                  ...
                  <div class="card-body py-4 px-5">
                    ...
                    <div class="row g-3">
                      ...
                      <div class="col-6">
                        ...
                        <span class="fw-semibold fs-7">{{ $pendapatan->tahun_anggaran ?? $tahunAnggaran }}</span>
                      </div>
                      <div class="col-6">
                        <span class="text-muted fs-8 d-block">Nilai Murni (sebelum)</span>
                        <span class="fw-semibold fs-7 text-info">
                          Rp {{ number_format($pendapatan->nilaimurni ?? 0, 0, ',', '.') }}
                        </span>
                      </div>
                      <div class="col-6">
                      ...
                      @if ($pendapatan->updateddate)
                        <div class="col-12">
                          <span class="text-muted fs-8 d-block">Terakhir Diperbarui</span>
                          <span class="fw-semibold fs-7">
                            {{ $pendapatan->updateddate }} {{ $pendapatan->updatedtime }}
                          </span>
                        </div>
                      @endif
                    </div>
          ...
          <div class="card-footer bg-light">
            <div class="d-flex justify-content-end gap-2">
              <a href="{{ route('pendapatan.rincian', $id_skpd) }}" class="btn btn-light">
                <i class="ki-outline ki-cross fs-2"></i>
                Batal
              </a>
              <button type="submit" class="btn btn-primary" id="btn_update_pendapatan">
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      function updatePreview(raw) {
        var preview = document.getElementById('preview_nilai');
        if (!preview) return;
        var num = parseFloat(raw) || 0;
        preview.textContent = num > 0 ? 'Terbaca: Rp ' + num.toLocaleString('id-ID') : '';
      }

      var inputNilai = document.getElementById('input_nilai');
      ...
      if (inputNilai) {
        ...
        var form = document.getElementById('form_edit_pendapatan');
        if (form) {
          form.addEventListener('submit', function() {
            inputNilai.value = toNumber(inputNilai.value);
            var btn = document.getElementById('btn_update_pendapatan');
            if (btn) {
